...
class Model {
    ...
    template<class T, class ...Args> T* create_one(std::vector<SafePointer<T>>& t_destination, Args... t_args);
...
public:
    ...
    Variable<PlayerT> create_variable(double t_lower_bound, double t_upper_bound, VariableType t_type, std::string t_name = "");
    Variable<opp_player_v<PlayerT>> create_parameter(double t_lower_bound, double t_upper_bound, VariableType t_type, std::string t_name = "");
...
T* Model<PlayerT>::create_one(std::vector<SafePointer<T>> &t_destination, Args... t_args) {
    const unsigned int index = t_destination.size();
    auto* ptr_to_impl = ((ObjectCreator&) m_env).create<impl::Variable>(index, std::forward<Args>(t_args)...);
    t_destination.emplace_back(ptr_to_impl);
    return ptr_to_impl;
}

template<enum Player PlayerT>
Variable<PlayerT> Model<PlayerT>::create_variable(double t_lower_bound, double t_upper_bound, VariableType t_type, std::string t_name) {
    return Variable<PlayerT>(
            create_one<impl::Variable>(m_variables, t_lower_bound, t_upper_bound, t_type, t_name)
    );
}

template<enum Player PlayerT>
Variable<opp_player_v<PlayerT>>
Model<PlayerT>::create_parameter(double t_lower_bound, double t_upper_bound, VariableType t_type, std::string t_name) {
    return Variable<opp_player_v<PlayerT>>(
            create_one<impl::Variable>(m_parameters, t_lower_bound, t_upper_bound, t_type, t_name)
    );
}
