...
class Solver2 : public Algorithm {
    Model& m_src_model;
...
public:
    explicit Solver2(Model& t_model);
...
template<class VarT, class CtrT>
Solver2<VarT, CtrT>::Solver2(Model &t_model) : m_src_model(t_model) {

}

template<class VarT, class CtrT>
void Solver2<VarT, CtrT>::add_variable_impl(const VarT &t_var) {
    m_variables.template emplace_back(t_var);
}

template<class VarT, class CtrT>
void Solver2<VarT, CtrT>::add_constraint_impl(const CtrT &t_ctr) {
    m_constraints.template emplace_back(t_ctr);
}

template<class VarT, class CtrT>
void Solver2<VarT, CtrT>::remove_variable_impl(const Var &t_var) {
    m_variables[t_var.index()] = std::move(m_variables.back());
    m_variables.pop_back();
}

template<class VarT, class CtrT>
void Solver2<VarT, CtrT>::remove_constraint_impl(const Ctr &t_ctr) {
    m_constraints[t_ctr.index()] = std::move(m_constraints.back());
    m_constraints.pop_back();
}

template<class VarT, class CtrT>
double Solver2<VarT, CtrT>::value(const Constant &t_constant) const {
    // TODO uncomment this
