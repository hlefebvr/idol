...
Ctr::Ctr(Env &t_env, CtrType t_type, Constant&& t_constant, std::string t_name) : Ctr(t_env, TempCtr(Row(0., std::move(t_constant)), t_type), std::move(t_name)) {

}

Ctr::Ctr(Env &t_env, CtrType t_type, const Constant &t_constant, std::string t_name) : Ctr(t_env, t_type, Constant(t_constant), std::move(t_name)) {
