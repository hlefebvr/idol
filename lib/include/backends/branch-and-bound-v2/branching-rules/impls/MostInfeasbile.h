...
protected:
    ...
    virtual double most_infeasible_score(const Var& t_var, const Node2<NodeInfoT>& t_node) const;

    virtual void select_integer_variable_for_branching(const Node2<NodeInfoT>& t_node);
    virtual std::list<NodeInfoT *> create_child_nodes_by_bound_for_integer_variable(
            const Node2<NodeInfoT> &t_node,
            const Var& t_var,
            double t_value);
public:
    ...
    bool is_valid(const Node2<NodeInfoT> &t_node) const override;

    [[nodiscard]] std::list<NodeInfoT*> create_child_nodes(const Node2<NodeInfoT>& t_node) override;
};

template<class NodeInfoT>
std::list<NodeInfoT *> BranchingRules::MostInfeasible<NodeInfoT>::create_child_nodes_by_bound_for_integer_variable(
        const Node2<NodeInfoT> &t_node, const Var &t_var, double t_value) {
...
template<class NodeInfoT>
void BranchingRules::MostInfeasible<NodeInfoT>::select_integer_variable_for_branching(const Node2<NodeInfoT> &t_node) {
...
double BranchingRules::MostInfeasible<NodeT>::most_infeasible_score(const Var &t_var, const Node2<NodeT> &t_node) const {
...
template<class NodeT>
bool BranchingRules::MostInfeasible<NodeT>::is_valid(const Node2<NodeT> &t_node) const {
    ...
    for (const auto& var : m_branching_candidates) {
        if (double value = primal.get(var) ; !is_integer(value)) {
            idol_Log(Trace, BranchAndBoundOptimizer<NodeT>, "Node " << t_node.id() << " solution not valid (" << var << " = " << value << ")." );
            return false;
        }
    }
...
template<class NodeT>
std::list<NodeT *>
BranchingRules::MostInfeasible<NodeT>::create_child_nodes(const Node2<NodeT> &t_node) {
