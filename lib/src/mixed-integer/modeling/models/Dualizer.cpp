...
idol::Dualizer::Dualizer(const idol::Model &t_parent,
    ...
    if (!primal_is_a_linear_problem()) {
        throw Exception("Dualizer only supports linear problems.");
    }
...
std::pair<double, double> idol::Dualizer::bounds_for_dual_variable(idol::CtrType t_type) const {
    switch (t_type) {
...
void idol::Dualizer::create_dual_variables() {
...
bool idol::Dualizer::primal_is_a_linear_problem() const {
...
bool idol::Dualizer::is_linear(const QuadExpr<Var, double>& t_expr) const {
...
void idol::Dualizer::create_dual_constraints() {
...
void idol::Dualizer::add_dual(idol::Model &t_destination) {
    add_dual_variables(t_destination);
    add_dual_constraints(t_destination);
    add_dual_objective(t_destination);
}

void idol::Dualizer::add_dual_variables(idol::Model &t_destination) {
...
void idol::Dualizer::add_dual_constraints(idol::Model &t_destination) {
...
void idol::Dualizer::add_dual_objective(idol::Model &t_destination) {
    t_destination.set_obj_expr(t_destination.get_obj_expr() + m_dual_objective);
    t_destination.set_obj_sense(Maximize);
}

void idol::Dualizer::add_coupling(idol::Model &t_destination) {
    add_coupling_variables(t_destination);
    add_coupling_constraints(t_destination);
}

void idol::Dualizer::add_coupling_variables(idol::Model &t_destination) {
...
void idol::Dualizer::add_coupling_constraints(idol::Model &t_destination) {
...
void idol::Dualizer::add_primal(idol::Model &t_destination) {
    add_primal_variables(t_destination);
    add_primal_constraints(t_destination);
    add_primal_objective(t_destination);
}

void idol::Dualizer::add_primal_variables(idol::Model &t_destination) {
    for (const auto& var : m_primal.vars()) {
        const double lb = m_primal.get_var_lb(var);
        const double ub = m_primal.get_var_ub(var);
        const auto type = m_primal.get_var_type(var);
...
void idol::Dualizer::add_primal_constraints(idol::Model &t_destination) {
    for (const auto& ctr : m_primal.ctrs()) {
        const auto& row = m_primal.get_ctr_row(ctr);
        const double rhs = m_primal.get_ctr_rhs(ctr);
        const auto type = m_primal.get_ctr_type(ctr);
...
void idol::Dualizer::add_primal_objective(idol::Model &t_destination) {
    t_destination.set_obj_expr(m_primal_objective);
    t_destination.set_obj_sense(Minimize);
}

void idol::Dualizer::add_strong_duality_reformulation(idol::Model &t_destination) {
    add_primal_variables(t_destination);
    add_primal_constraints(t_destination);
    add_dual_variables(t_destination);
    add_dual_constraints(t_destination);
...
void idol::Dualizer::add_kkt_reformulation(idol::Model &t_destination) {
    add_primal_variables(t_destination);
    add_primal_constraints(t_destination);
    add_dual_variables(t_destination);
    add_dual_constraints(t_destination);
