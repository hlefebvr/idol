...
namespace BranchingStrategies {
    class BranchingStrategies_MostInfeasible;
}

class BranchingStrategies::BranchingStrategies_MostInfeasible {
public:
    template<class NodeT> class Strategy;
};

template<class NodeT>
class BranchingStrategies::BranchingStrategies_MostInfeasible::Strategy : public BranchingStrategyWithNodeType<NodeT> {
...
template<class NodeT>
bool BranchingStrategies::BranchingStrategies_MostInfeasible::Strategy<NodeT>::is_valid(const NodeT &t_node) const {
...
template<class NodeT>
std::list<NodeT *> BranchingStrategies::BranchingStrategies_MostInfeasible::Strategy<NodeT>::create_child_nodes(const NodeT &t_node, const std::function<unsigned int()>& t_id_provider) const {
...
template<class NodeT>
double BranchingStrategies::BranchingStrategies_MostInfeasible::Strategy<NodeT>::fractional_part(double t_x) {
    return std::abs(std::round(t_x) - t_x);
}

template<class NodeT>
bool BranchingStrategies::BranchingStrategies_MostInfeasible::Strategy<NodeT>::is_integer(double t_x) {
    return fractional_part(t_x) <= ToleranceForIntegrality;
}

template<class NodeT>
double BranchingStrategies::BranchingStrategies_MostInfeasible::Strategy<NodeT>::score(const Var &t_var, const NodeT &t_node) const {
