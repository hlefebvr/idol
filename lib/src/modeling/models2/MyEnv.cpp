...
void impl::MyEnv::remove_version(const MyModel &t_model, const MyCtr &t_ctr) {
    t_ctr.remove_version(t_model);
}
...
const CtrVersion &impl::MyEnv::version(const MyModel &t_model, const MyCtr &t_ctr) const {
    return t_ctr.versions().get(t_model);
}

CtrVersion &impl::MyEnv::version(const MyModel &t_model, const MyCtr &t_ctr) {
    return const_cast<MyCtr&>(t_ctr).versions().get(t_model);;
}
...
MyCtr impl::MyEnv::create_ctr(std::string t_name) {
