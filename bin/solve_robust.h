...
public:
    ...
    MethodManager(storage&& t_all_methods) : m_all_methods(std::move(t_all_methods)) {}
    ...
    void print_available_methods(bool t_with_details) {
        ...
        bool first = true;
        for (auto it = m_available_methods.rbegin(); it != m_available_methods.rend(); ++it) {
            if (!first) {
                std::cout << ", ";
            }
        ...
        if (t_with_details) {
            ...
            for (auto it = m_available_methods.rbegin(); it != m_available_methods.rend(); it++) {
                std::cout << "\t\t " << it->second->first << ": " << it->second->second.second << "\n";
            }
    ...
    const std::string& get_default_method() const {
