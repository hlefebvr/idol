...
idol::DantzigWolfeDecomposition::DantzigWolfeDecomposition(idol::Annotation<unsigned int> t_decomposition)
...
idol::Optimizer *idol::DantzigWolfeDecomposition::operator()(const Model &t_model) const {
    ...
    if (!m_decomposition.has_value()) {
        throw Exception("No decomposition has been configured.");
    }

    DantzigWolfe::Formulation dantzig_wolfe_formulation(t_model,m_decomposition.value());
